...
export function migrateMessage(
  message: Omit<Message, 'contentParts'> & {
  ...
  if (
    (!result.contentParts?.length ||
      getMessageText(result) === '...' ||
      !getMessageText(result)) &&
    'content' in message
  ) {
    const imageParts = (
      message as Message & { pictures?: MessagePicture[] }
    ).pictures
      ?.filter((pic) => pic.storageKey || pic.url)
      .map((pic) => ({
        type: 'image' as const,
        storageKey: pic.storageKey!,
        url: pic.url,
      }));
    result.contentParts = [
...
export function fixMessageRoleSequence(messages: Message[]): Message[] {
  ...
  if (
    ...
    result[firstAssistantIndex - 1]?.role !== 'user'
  ) {
    result = [
